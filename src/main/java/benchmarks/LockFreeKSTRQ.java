...
public class LockFreeKSTRQ<E extends Comparable<? super E>, V> implements CompositionalMap<E,V> {
    ...
    {
        if(node == null) return 0;
        if(node.c == null) return countCurrNode(node);

        int sum = 0;

        for(int i = 0; i < node.c.length(); ++i)
        {
            sum+=countNodes((Node)node.c.get(i));
        }

        return sum;
    }

    private int countCurrNode(Node node) {
        if(node == null) return 0;
        int sum = 0;

        if(node.k != null) sum += node.k.length;
        if(node.v != null) sum += node.v.length;
        return sum;
    }
...
//        for (int innerSum=0;innerSum<l.kcount;innerSum++) {
//            if (less(key, (E)l.k[innerSum])) return l.c.get(innerSum);
//        }
//        return l.c.get(l.kcount);
    ...
    public static final class Node<E extends Comparable<? super E>, V> {
        ...
        public Node(final E knew, final Object vnew, final Node<E,V> l) {
            ...
            // determine which elements of l.k should precede knew (will be 0...innerSum-1)
            int i = 0;
            for (;i<kcount;i++) {
            ...
            if (haskey) {
                ...
                for (;i<l.kcount;i++) {
                    if (less(knew, (E)l.k[i])) {
                        break; // l.k[0...innerSum-1] will all precede knew.
                    }
                }

                // write <key,val> pairs preceding <knew,vnew>
                // knew precedes l.k[innerSum], but all of l.k[0...innerSum-1] precede knew.
                for (int j=0;j<i;j++) {
                    this.k[j] = l.k[j];
                    this.v[j] = l.v[j];
                }
        ...
        public Node(final E key, final Node<E,V> l) {
            ...
            for (int i=0;i<l.kcount;i++) {
                if (equal(key, (E)l.k[i])) {
                    // key innerSum is being removed from l
                    // so everything in l.k[0...innerSum-1] union l.k[innerSum+1...l.kcount-1] remains
                    for (int j=0;j<i;j++) {
                        this.k[j] = l.k[j];
                        this.v[j] = l.v[j];
                    }
        ...
        public String toString() {
            // removed to enable debug information for ordered fill
